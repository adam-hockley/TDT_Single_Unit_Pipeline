    ...
    for pos = 1:length(Positions)
        ...
        if exist([tank_path '\StreamSplitInfo_All.mat'])
            ...
            if exist([tank_path '\Sorting\' Sorter '\Phy\spike_times.npy'])
                ...
                KSLabels = readtable([tank_path '\Sorting\' Sorter '\Phy\cluster_KSLabel.tsv'], "FileType","text",'Delimiter', '\t');
                KSLabels.cluster_id = KSLabels.cluster_id+1;
                ...
                if ~isempty(goodnums)
                    ...
                    templateindexes = readNPY([tank_path '\Sorting\' Sorter '\Phy\templates_ind.npy']); % Spike shapes
                    clear bestchannel amp SpikeShapesNew

                    newclusters = unique(PhyRez(:,2));
                    newclusters = goodnums;
                    ...
                    for i = 1:length(StreamSplitInfo.Blocks) % loop the different blocks
                        ...
                        units = goodnums; % get list of unit numbers from sorted data
